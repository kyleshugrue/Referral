...
export class FirebaseStorageService {
  ...
  private async generatePdfPreviewsFromBuffer(pdfBuffer: Buffer, fileName: string, userId?: number, firebaseUid?: string): Promise<string[]> {
    ...
    try {
      // The directory is generated by mkdtemp and the filename is fixed. Use
      // one exclusive handle for the bounded, already-validated PDF bytes.
      const tempPdfHandle = await fs.promises.open(tempPdfPath, 'wx');
      try {
        await tempPdfHandle.writeFile(Buffer.from(pdfBuffer));
      } finally {
        await tempPdfHandle.close();
      }
